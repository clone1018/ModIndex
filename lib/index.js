...
function init() {
	...
	fieldSearch.onblur = uiDestroyTooltip;
}
...
function settingsInit() {
	...
	// Create the default settings skeleton
	settingsDefault = {
		'rowstyle' : {
			'latest' : {
				'clr_button' : false,
	...
	// Skeletons for other row style types
	settingsDefault.rowstyle.outdated = settingsDefault.rowstyle.latest;
	settingsDefault.rowstyle.unknown = settingsDefault.rowstyle.latest;
	settingsDefault.rowstyle.favorite = settingsDefault.rowstyle.latest;
...
function settingsUpdate() {
	settings = arrayMerge(settings, settingsDefault);
...
function settingsSave() {
	cookieWrite( 'settings', JSON.stringify(settings) );
	indexReload();
}
...
function uiRegisterCodeboxes() {
	...
	for (var i in codeboxes) {
		codeboxes[i].readOnly = true;
		codeboxes[i].onmouseover = function() {
			this.focus();
			this.select();
		}
		
		codeboxes[i].onmouseout = function() {
			this.blur();
		}
	}
}
...
function uiRegisterCheckboxes() {
	...
	for (var i in checkboxes) {
		checkboxes[i].checkbox = checkboxes[i].firstElementChild;
...
function editorShow(e) {
	...
	for (var i in checkboxesEditorFlags)
		checkboxesEditorFlags[i].checked = false;
...
function editorInit() {
	...
	for (var i in ENUM_FLAGS)
		checkboxesEditorFlags[i] = document.getElementsByName('checkbox_editor_flag_' + i)[0];
}
...
function editorCallback(data, id) {
	...
	for (var i in checkboxesEditorFlags)
		checkboxesEditorFlags[i].checked = row['flag_'+i];
}
...
function arrayMerge(a, b) {
	...
	for (element in b) {
		...
		if (typeof(a[element]) != 'undefined')
			newArray[element] = a[element];
		else
			newArray[element] = b[element];
